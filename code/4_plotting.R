...
ktnescplot2024 <- ktn_survey_final %>% 
  group_by(year) %>% 
...
ktnseparatedplot2024 <- ktn_survey_final %>% 
  ...
  labs(x = "", y = "Escapement Survey Count", 
       ...
       fill = "Stream") + 
  theme_crisp(base_family = "Arial")
ktnseparatedplot2024

ktnfacetplot2024 <- ktn_survey_final %>% 
  ggplot(aes(x = year, y = total_count, fill = stream_name)) + 
  geom_col(color = "black") +
  scale_fill_adfg(palette = "camai") +
...
ggsave(ktnescplot2024,       filename = "plots/2024_KTN_1_EscSummary.png",     dpi = 700)
ggsave(ktnseparatedplot2024, filename = "plots/2024_KTN_2_AllRiversTotal.png", dpi = 700)
ggsave(ktnfacetplot2024,     filename = "plots/2024_KTN_3_AllRiversFacet.png", dpi = 700, width = 12, height = 8)
